...
/// <summary>
/// Timer-triggered function that polls Azure OpenAI for batch job status updates.
/// Runs every 5 minutes by default (configurable via BatchPollingSchedule).
/// </summary>
public class BatchPollingFunction
{
    ...
    private readonly ILogger<BatchPollingFunction> _logger;

    public BatchPollingFunction(
        ...
        ILogger<BatchPollingFunction> logger)
    {
    ...
    [Function("BatchPollingFunction")]
    public async Task Run([TimerTrigger("%BatchPollingSchedule%")] TimerInfo timerInfo)
    {
        _logger.LogInformation("BatchPollingFunction triggered at: {Time}", DateTime.UtcNow);

        try
        {
            await PollBatchResultsAsync();
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error in BatchPollingFunction");
            throw;
        }

        _logger.LogInformation("BatchPollingFunction completed. Next scheduled run: {NextRun}",
            timerInfo.ScheduleStatus?.Next);
    }

    private async Task PollBatchResultsAsync()
    {
        var submittedBatches = await _batchJobRepository.GetSubmittedJobsAsync();

        _logger.LogInformation("Found {Count} submitted batches to poll", submittedBatches.Count());
        ...
        foreach (var batch in submittedBatches)
        {
            ...
            try
            {
                await ProcessBatchResultAsync(batch);
            }
            catch (Exception ex)
            {
    ...
    private async Task ProcessBatchResultAsync(BatchJob batch)
    {
